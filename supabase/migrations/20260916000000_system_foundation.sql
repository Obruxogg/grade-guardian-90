...
-- Description: Core foundation helper RPC functions, bootstrap admin, safe teacher creation, system status, and updated RLS.
...
BEGIN
  ...
  INSERT INTO public.teachers (user_id, full_name, status)
  VALUES (_new_user_id, p_full_name, _status_enum)
  ON CONFLICT (user_id) DO UPDATE SET full_name = EXCLUDED.full_name, status = EXCLUDED.status;
